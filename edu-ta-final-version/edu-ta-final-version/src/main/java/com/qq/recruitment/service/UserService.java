...
public class UserService {
    ...
    public boolean register(String username, String password, String fullName, String role) {
        ...
        if (isFullNameTaken(finalFullName, null)) {
            return false;
        }
    ...
    public boolean updateFullName(String username, String fullName) {
        ...
        try {
            normalizedUsername = InputValidator.normalizeUsername(username);
            normalizedFullName = InputValidator.normalizeRequiredText(fullName, "Full name", 80, false);
        } catch (IllegalArgumentException ex) {
            return false;
        }
        ...
        if (isFullNameTaken(normalizedFullName, normalizedUsername)) {
            return false;
        }
        User user = userOpt.get();
    ...
    public boolean isFullNameTaken(String fullName, String excludedUsername) {
        if (fullName == null || fullName.isBlank()) {
            return false;
        }
        String normalizedFullName = fullName.trim();
        String normalizedExcludedUsername = excludedUsername == null ? null : excludedUsername.trim();
        return userDAO.getAllUsers().stream()
                .anyMatch(user -> user.getFullName() != null
                        && user.getFullName().trim().equalsIgnoreCase(normalizedFullName)
                        && (normalizedExcludedUsername == null
                        || !user.getUsername().equals(normalizedExcludedUsername)));
    }
}
