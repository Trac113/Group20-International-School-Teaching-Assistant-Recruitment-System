        ...
        userActionColumn.setCellFactory(param -> new TableCell<>() {
...

            {
                ...
                fullNameBtn.setOnAction(event -> {
                    ...
                    if (userService.isFullNameTaken(newName, user.getUsername())) {
                        showAlert(Alert.AlertType.WARNING, "Warning", "Full name already exists.");
                        return;
                    }
                    if (userService.updateFullName(user.getUsername(), newName)) {
                        ...
                        showAlert(Alert.AlertType.ERROR, "Error", "Failed to update full name. Full name may already exist.");
                    }
                });
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty) {
                    ...
                    pane.getChildren().addAll(passwordBtn, fullNameBtn);
                    if ("APPLICANT".equals(user.getRole())) {
                        pane.getChildren().add(workloadBtn);
                    }
                    setGraphic(pane);
                }
            }
